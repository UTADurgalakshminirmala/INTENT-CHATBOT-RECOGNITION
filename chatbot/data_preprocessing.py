...
# Select the first 100 rows and the "question" and "answer" columns
...
def get_intent():
    ...
    # Check if the intent exists in your dataset
    if intent in data['answer'].values:
        return render_template('index.html', user_question=user_question, intent=intent)
    else:
        return render_template('index.html', user_question=user_question, intent="Intent not found in dataset")
